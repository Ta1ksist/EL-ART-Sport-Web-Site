...
export async function getProjectDescription(slug: string): Promise<DescriptionImage[]> {
  try {
    const result = await cloudinary.search
      .expression(`folder:${slug}`)
      .max_results(100)
    ...
    const descriptionResources = resources.filter((resource: any) => {
      const filename = resource.public_id.split('/').pop() || '';
      return filename.startsWith('description');
    });

    const sorted = descriptionResources.sort((a: any, b: any) => {
      const nameA = a.public_id.split('/').pop() || '';
      const nameB = b.public_id.split('/').pop() || '';
    ...
    console.error(`Ошибка загрузки для ${slug}:`, error);
    return [];
  }
}
